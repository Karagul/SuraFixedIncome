ns_optim_pr_reg <-
function(beta_ini, date_ini, mk_price, coupon, matur, freq, conv, y_lim=c(0,5), delta_beta=0.01, min_beta=c( 0 , -15 , -10 ,5), max_beta=c(15 , 0, 10 ,10)){
  
  diff<-function(betas,date_ini, mk_price, coupon, matur, freq, conv, y_lim){
    res=diff_pr_ns(betas, date_ini, mk_price, coupon, matur, freq, conv, y_lim)+delta_beta*crossprod(betas-betas_ini)
    return(res)
  }
