cc_curve_index <- function(cc_hist, index_ini = 1000, dates = NULL, target_matur_in_days = 1095, bond_freq = 2, base = 365){
  
  cc_curve <- na.omit(cc_hist[, as.numeric(colnames(cc_hist)) <= target_matur_in_days])
  if(!is.null(dates)){
    cc_curve[paste(dates, collapse = '/')]  
  }
  ...
  for(i in 2:length(cc_dates)){
    ...
    bond_pr <- sum(cf * df) 
    index_val[i] <- index_val[i-1] * bond_pr
